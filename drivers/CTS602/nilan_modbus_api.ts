...
import {ModbusResultParameters,RegisterType} from "./constants";

const {default: PQueue} = require('p-queue');

export interface NilanApiOptions {
  device?: Device;
  homey: any;
  logger: any;
...
export class NilanApi {
  ...
  constructor(options: NilanApiOptions) {
  ...
  async getDeviceType() {
    ...
    if (client) {
      try {
        let res = await client.readHoldingRegisters(1000, 1);
        return res.response.body.valuesAsBuffer.readInt16BE();
      } catch(error) {
  ...
  async read(params: any) {
    return this._commandQueue.add(async () => {
        const client = await this._connection();
        if (client) {
          Promise.all(params.map((p: any) => {
              if (p.regType === RegisterType.Input) return client.readInputRegisters(p.register, 1);
              else if (p.regType === RegisterType.Holding) return client.readHoldingRegisters(p.register, 1);
              else if (p.regType === RegisterType.Coil) return client.readCoils(p.register, 1);
              else if (p.regType === RegisterType.Discrete) return client.readDiscreteInputs(p.register, 1);
            }))
            .then((results) => {
        ...
        if (client) {
          Promise.all(params
            .map((param: any, i: number) => {
              ...
              return client.writeMultipleRegisters(param.register, [toVal]);
              //return client.writeSingleRegister(param.register, toVal);
            }))
            .then((responses) => {
